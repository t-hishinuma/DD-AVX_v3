...
public:
  ...
  dd_real_vector_ref(typename std::vector<double>::iterator hi,
                     typename std::vector<double>::iterator lo)
      : dd_hi_iter(hi), dd_lo_iter(lo) {}

  operator double() const { return *dd_hi_iter; }

  operator dd_real() const { return dd_real(*dd_hi_iter, *dd_lo_iter); }
