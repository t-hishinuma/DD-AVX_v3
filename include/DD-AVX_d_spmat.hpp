...
public:
   ...
   * @param row_pointer which stores the starting points of the rows of the
   *arrays value and col_ind (size M+1)
   * @param col_index which stores the column numbers of the non-zero elements
   *(size nnz)
   * @param value which stores the non-zero elements (size nnz)
   **/
  d_real_SpMat(int r, int c, int NNZ, int *row_pointer, int *col_index,
               double *value) {
    if (r != c) {
      std::cerr << "error, r!=c, square matrix only now" << std::endl;
      assert(1);
   ...
   * @param value which stores the non-zero elements (size nnz)
   **/
  d_real_SpMat(std::vector<int> &row_pointer, std::vector<int> &col_index,
   ...
   * @param r # of row
   * @param row_index which stores the row numbers of the non-zero elements
   *(size nnz)
   * @param col_index which stores the column numbers of the non-zero elements
   *(size nnz)
   * @param value which stores the non-zero elements (size nnz)
   **/
  d_real_SpMat(int r, std::vector<int> &row_index, std::vector<int> &col_index,
               std::vector<double> &value) {
  ...
  std::vector<double>::reference data() { return val[0]; }
