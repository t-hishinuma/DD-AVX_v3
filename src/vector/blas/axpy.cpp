...
#pragma omp parallel private(regs)
  {
    ...
    for (; i < ie; i++) {
      Fma(y.hi[i], y.lo[i], y.hi[i], y.lo[i], alpha.x[0], alpha.x[1], x.hi[i],
          x.lo[i]);
    }
  }
}
...
#pragma omp parallel private(regs)
  {
    ...
    for (; i < ie; i++) {
      Fmad(y.hi[i], y.lo[i], y.hi[i], y.lo[i], alpha.x[0], alpha.x[1],
           x.data()[i]);
    }
  }
}
...
#pragma omp parallel private(regs)
  {
    ...
    for (; i < ie; i++) {
      Fma(y.data()[i], y.data()[i], 0.0, alpha.x[0], alpha.x[1], x.hi[i],
          x.lo[i]);
    }
  }
}
...
#pragma omp parallel private(regs)
  {
    ...
    for (; i < ie; i++) {
      Fma(y.data()[i], y.data()[i], 0.0, alpha.x[0], alpha.x[1], x.data()[i],
          0.0);
    }
  }
}
