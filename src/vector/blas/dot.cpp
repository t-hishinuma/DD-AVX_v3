...
#pragma omp parallel private(regs)
  {
    ...
    for (; i < ie; i++) {
      Fma(dot[thN].x[0], dot[thN].x[1], dot[thN].x[0], dot[thN].x[1], x.hi[i],
          x.lo[i], y.hi[i], y.lo[i]);
    }
  }
...
#pragma omp parallel private(regs)
  {
    ...
    for (; i < ie; i++) {
      Fmad(dot[thN].x[0], dot[thN].x[1], dot[thN].x[0], dot[thN].x[1], y.hi[i],
           y.lo[i], x.data()[i]);
    }
  }
...
#pragma omp parallel private(regs)
  {
    ...
    for (; i < ie; i++) {
      Fmad(dot[thN].x[0], dot[thN].x[1], dot[thN].x[0], dot[thN].x[1], x.hi[i],
           x.lo[i], y.data()[i]);
    }
  }
...
#pragma omp parallel private(regs)
  {
    ...
    for (; i < ie; i++) {
      Fmad(dot[thN].x[0], dot[thN].x[1], dot[thN].x[0], dot[thN].x[1],
           x.data()[i], 0.0, y.data()[i]);
    }
  }
