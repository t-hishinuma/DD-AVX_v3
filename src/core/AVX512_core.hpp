...
// add
inline reg add(const reg a, const reg b) { return SIMD_FUNC(add_pd)(a, b); }

// mul
inline reg mul(const reg a, const reg b) { return SIMD_FUNC(mul_pd)(a, b); }

// sub
inline reg sub(const reg a, const reg b) { return SIMD_FUNC(sub_pd)(a, b); }

// div
inline reg div(const reg a, const reg b) { return SIMD_FUNC(div_pd)(a, b); }
...
// need to change for AVX512
inline void store(double &ret1, double &ret2, double &ret3, double &ret4,
                  double &ret5, double &ret6, double &ret7, double &ret8,
                  const reg val) {
  double tmp[8];
  store(*tmp, val);
  ret1 = tmp[0];
...
inline void to_minus(reg &reg, registers &regs) { reg = mul(reg, regs.minus); }
} // namespace ddavx_core
#endif
